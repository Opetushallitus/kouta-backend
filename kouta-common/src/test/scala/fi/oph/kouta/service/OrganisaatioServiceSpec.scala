...
import fi.oph.kouta.domain.{Amm, Koulutustyyppi, Lk, Tuva, VapaaSivistystyoMuu, VapaaSivistystyoOpistovuosi, Yo}
...
class OrganisaatioServiceSpec extends OrganisaatioFixture {
  ...
  it should "return a flat list of oppilaitostyyppi present in parents and children" in {
    organisaatioService.getAllChildOidsAndOppilaitostyypitFlat(ChildOid)._2 should contain theSameElementsAs List(Amm, Tuva, VapaaSivistystyoOpistovuosi, VapaaSivistystyoMuu)
  }
  ...
  it should "return a flat list of oppilaitostyyppi present in parents and children 3" in {
    organisaatioService.getAllChildOidsAndOppilaitostyypitFlat(GrandChildOid)._2 should contain theSameElementsAs List(Amm, Tuva, VapaaSivistystyoOpistovuosi, VapaaSivistystyoMuu)
  }
