...
object Koulutustyyppi extends Enum[Koulutustyyppi] {
  ...
  def values =
    List(
      ...
      VapaaSivistystyoMuu,
      AmmOpeErityisopeJaOpoKoulutus
    )

  def ammatilliset           = List(Amm, AmmTutkinnonOsa, AmmOsaamisala)
  def korkeakoulu            = List(Amk, Yo, AmmOpeErityisopeJaOpoKoulutus)
  ...
  val oppilaitostyyppi2koulutustyyppi: Map[String, Seq[Koulutustyyppi]] = Map(
    ...
    "oppilaitostyyppi_41#1" -> Seq(Amk, AmmOpeErityisopeJaOpoKoulutus), //Ammattikorkeakoulut
    ...
    "oppilaitostyyppi_46#1" -> Seq(Amk, AmmOpeErityisopeJaOpoKoulutus), //Väliaikaiset ammattikorkeakoulut
...
case object AmmOpeErityisopeJaOpoKoulutus extends Koulutustyyppi { val name = "ammatillinen_ope_erityisope_ja_opokoulutus" }
