...
trait AuthorizationRuleForJulkinen extends AuthorizationRule {
  def isJulkinenAuthorized(
      entity: AuthorizableMaybeJulkinen[_],
      additionalOrganisaatioOids: Seq[OrganisaatioOid],
      usersOidsAndOppilaitostyypit: OrganisaatioOidsAndOppilaitostyypitFlatView,
      userIsMemberOfOwnerOrg: Boolean
  ): Boolean

  def assertUserAuthorizedToKoulutustyyppi(
      entity: AuthorizableMaybeJulkinen[_],
      usersOidsAndOppilaitostyypit: OrganisaatioOidsAndOppilaitostyypitFlatView
  ): Boolean = {
    if (!usersOidsAndOppilaitostyypit.exists(_._2.contains(entity.koulutustyyppi)))
      throw KoulutustyyppiAuthorizationFailedException(
        entity.koulutustyyppi,
        usersOidsAndOppilaitostyypit.map(_._2).toSeq.flatten.distinct
      )
    true
  }
    ...
    authorizable match {
      ...
      case _ =>
        throw new RuntimeException(
          "AuthorizationRuleForJulkinen kutsuttu entitylla, joka ei ole AuthorizableMaybeJulkinen"
        )
    }
}

case object AuthorizationRuleByOrganizationAndKoulutustyyppi extends AuthorizationRuleForJulkinen {
  override def isJulkinenAuthorized(
      entity: AuthorizableMaybeJulkinen[_],
      additionalOrganisaatioOids: Seq[OrganisaatioOid],
      usersOidsAndOppilaitostyypit: OrganisaatioOidsAndOppilaitostyypitFlatView,
      userIsMemberOfOwnerOrg: Boolean
  ): Boolean = {
    authorizedOrganisations(entity, additionalOrganisaatioOids).forall(org =>
      usersOidsAndOppilaitostyypit.exists(_._1.exists(_ == org))
    ) match {
  ...
  override def authorizedOrganisations(
      entity: Authorizable,
      additionalOrganisaatioOids: Seq[OrganisaatioOid]
  ): Seq[OrganisaatioOid] =
...
case object AuthorizationRuleForReadJulkinen extends AuthorizationRuleForJulkinen {
  override def isJulkinenAuthorized(
    ...
    AuthorizedToAnyOfGivenOrganizationsRule.isAuthorized(
      entity,
      additionalOrganisaatioOids,
      usersOidsAndOppilaitostyypit,
      userIsMemberOfOwnerOrg: Boolean
    ) match {
      case false => entity.julkinen && assertUserAuthorizedToKoulutustyyppi(entity, usersOidsAndOppilaitostyypit)
      case true if entity.organisaatioOid == RootOrganisaatioOid =>
        entity.julkinen && assertUserAuthorizedToKoulutustyyppi(entity, usersOidsAndOppilaitostyypit)
      case _ => true
    }
...
case object AuthorizationRuleByKoulutustyyppi extends AuthorizationRule {
  override def isAuthorized(
    ...
    authorizable match {
      ...
      case _ =>
        throw new RuntimeException(
          "authorizationRuleByKoulutustyyppi kutsuttu entitylla, joka ei ole AuthorizableByKoulutustyyppi"
        )
    }
  override def organizationsAuthorizationMode(): OrganizationsAuthorizationMode = Any
  override def authorizedOrganisations(
...
trait RoleEntityAuthorizationService[E <: AuthorizableEntity[E]] extends AuthorizationService {
  ...
  def authorizeGet(entityWithTime: Option[(E, Instant)])(implicit authenticated: Authenticated): Option[(E, Instant)] =
    authorizeGet(entityWithTime, AuthorizationRules(roleEntity.readRoles))

  def authorizeGet(entityWithTime: Option[(E, Instant)], authorizationRules: AuthorizationRules)(implicit
      authenticated: Authenticated
  ): Option[(E, Instant)] =
    entityWithTime.map { case (e, t) =>
      (authorizeGet(e, authorizationRules), t)
    }
  ...
  def authorizeGet(entity: E, authorizationRules: AuthorizationRules)(implicit authenticated: Authenticated): E =
    ifAuthorized(entity, authorizationRules)(entity)

  def authorizeGetWithType[T <: AuthorizableEntity[T]](entity: T)(implicit authenticated: Authenticated): T =
    authorizeGetWithType(entity, AuthorizationRules(roleEntity.readRoles))

  def authorizeGetWithType[T <: AuthorizableEntity[T]](entity: T, authorizationRules: AuthorizationRules)(implicit
      authenticated: Authenticated
  ): T =
    ifAuthorized(entity, authorizationRules)(entity)
  ...
  private def checkRuleList[I](rule: AuthorizationRules, rules: List[AuthorizationRules], entity: E)(
      f: => I
  )(implicit authenticated: Authenticated): I = {
    if (rules.isEmpty) {
  ...
  def authorizePut[I](entity: E, authorizationRules: AuthorizationRules = AuthorizationRules(roleEntity.createRoles))(
      f: E => I
  )(implicit authenticated: Authenticated): I =
  ...
  def authorizePut[I](entity: E, authorizationRules: List[AuthorizationRules])(
      f: E => I
  )(implicit authenticated: Authenticated): I =
  ...
  def authorizeUpdate[I](
      oldEntity: => Option[(E, Instant)],
      updatedEntity: E,
  ...
  private def doAuthorizeUpdate[I](oldEntity: E, updatedEntity: E, authorizationRules: List[AuthorizationRules])(
      f: (E, E) => I
  )(implicit authenticated: Authenticated): I = {
    ...
    authorizationRules match {
      case Nil =>
        if (organisaatioOidChanged())
          checkRuleList(organisaatioOidChangeRules(), List(), oldEntity)(
            f(oldEntity, withUpdatedMuokkaaja(updatedEntity))
          )
      ...
      case rules =>
        ...
        checkRuleList(rulesForUpdate.head, rulesForUpdate.tail, oldEntity)(
          f(oldEntity, withUpdatedMuokkaaja(updatedEntity))
        )
    }
  }
