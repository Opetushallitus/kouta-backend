...
class OrganisaatioServlet(organisaatioService: OrganisaatioService) extends KoutaServlet {
  ...
  get("/:oid") {
    ...
    organisaatioService.getOrganisaatio(OrganisaatioOid(params("oid"))) match {
      case Right(organisaatio: Organisaatio) => Ok(OppilaitosServiceUtil.organisaatioToKoutaOrganisaatio(organisaatio))
  ...
  post("/organisaatiot") {
    ...
    organisaatioService.getOrganisaatiot(parsedBody.extract[Seq[OrganisaatioOid]]) match {
      ...
      case Right(organisaatiot) => Ok(organisaatiot.map(org => OppilaitosServiceUtil.organisaatioToKoutaOrganisaatio(org)))
      case Left(e) => throw e
    }
  }
