        ...
        client.fetch(Request(method = GET, uri = url)) {
          ...
          case r =>
            r.bodyAsText
              .runLog
              .map(_.mkString)
              .flatMap(_ => Task.fail(new RuntimeException(s"Failed to fetch valintatapajono from valintaperusteet-service for valintatapajono-oid $valintatapajonoOid, http status ${r.status.code}")))
        }
