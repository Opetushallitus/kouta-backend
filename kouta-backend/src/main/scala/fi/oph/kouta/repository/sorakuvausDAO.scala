...
sealed trait SorakuvausSQL extends SorakuvausExtractors with SorakuvausModificationSQL with SQLHelpers {
  ...
  def selectByCreatorAndNotOph(organisaatioOids: Seq[OrganisaatioOid], myosArkistoidut: Boolean): DBIO[Vector[SorakuvausListItem]] = {
    sql"""select id, nimi, tila, organisaatio_oid, muokkaaja, lower(system_time)
          from sorakuvaukset
          where ( organisaatio_oid in (#${createOidInParams(organisaatioOids)}) and organisaatio_oid <> ${RootOrganisaatioOid}) #${createArkistoidutFilter(myosArkistoidut)}""".as[SorakuvausListItem]
  }
  ...
  def selectByCreatorOrJulkinenForKoulutustyyppi(organisaatioOids: Seq[OrganisaatioOid], koulutustyypit: Seq[Koulutustyyppi], myosArkistoidut: Boolean): DBIO[Vector[SorakuvausListItem]] = {
    sql"""select id, nimi, tila, organisaatio_oid, muokkaaja, lower(system_time)
          ...
          or (julkinen  = ${true} and koulutustyyppi in (#${createKoulutustyypitInParams(koulutustyypit)}))) #${createArkistoidutFilter(myosArkistoidut)}""".as[SorakuvausListItem]
  }
