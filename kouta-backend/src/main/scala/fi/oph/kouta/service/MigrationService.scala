...
trait MigrationHelpers extends Logging {
  ...
  def toLocalDateTime(time: Long): LocalDateTime =
  ...
  def parseLisatiedot(result: JValue): Seq[Lisatieto] = {
    Seq(
      (result \ "kuvausKomo" \ "KOULUTUKSEN_RAKENNE" \ "tekstis")
        .extractOpt[Map[String, String]]
        .map(k => Lisatieto("koulutuksenlisatiedot_01#1", toKieliMap(k))), // Opintojen rakenne
      (result \ "kuvausKomo" \ "TAVOITTEET" \ "tekstis")
        .extractOpt[Map[String, String]]
        .map(k => Lisatieto("koulutuksenlisatiedot_13#1", toKieliMap(k))), // Kohderyhmä
      (result \ "kuvausKomoto" \ "PAAAINEEN_VALINTA" \ "tekstis")
        .extractOpt[Map[String, String]]
        .map(k => Lisatieto("koulutuksenlisatiedot_03#1", toKieliMap(k))), // Suuntautumisvaihtoehdot
      (result \ "kuvausKomo" \ "PATEVYYS" \ "tekstis")
        .extractOpt[Map[String, String]]
        .map(k => Lisatieto("koulutuksenlisatiedot_09#1", toKieliMap(k))), // Koulutuksen antama pätevyys
      (result \ "kuvausKomoto" \ "SIJOITTUMINEN_TYOELAMAAN" \ "tekstis")
        .extractOpt[Map[String, String]]
        .map(k => Lisatieto("koulutuksenlisatiedot_04#1", toKieliMap(k))), // Uramahdollisuudet
      (result \ "kuvausKomoto" \ "KANSAINVALISTYMINEN" \ "tekstis")
        .extractOpt[Map[String, String]]
        .map(k => Lisatieto("koulutuksenlisatiedot_06#1", toKieliMap(k))), // Kansainvälistyminen
      (result \ "kuvausKomoto" \ "YHTEISTYO_MUIDEN_TOIMIJOIDEN_KANSSA" \ "tekstis")
        .extractOpt[Map[String, String]]
        .map(k => Lisatieto("koulutuksenlisatiedot_05#1", toKieliMap(k))), // Yhteistyö muiden toimijoiden kanssa
      (result \ "kuvausKomoto" \ "TUTKIMUKSEN_PAINOPISTEET" \ "tekstis")
        .extractOpt[Map[String, String]]
        .map(k => Lisatieto("koulutuksenlisatiedot_10#1", toKieliMap(k))), // Tutkimuksen painopisteet
      (result \ "kuvausKomoto" \ "LOPPUKOEVAATIMUKSET" \ "tekstis")
        .extractOpt[Map[String, String]]
        .map(k => Lisatieto("koulutuksenlisatiedot_11#1", toKieliMap(k))), // Opinnäytetyö
      (result \ "kuvausKomo" \ "JATKOOPINTO_MAHDOLLISUUDET" \ "tekstis")
        .extractOpt[Map[String, String]]
        .map(k => Lisatieto("koulutuksenlisatiedot_02#1", toKieliMap(k))) // Jatko-opintomahdollisuudet
  ...
  def toKoulutuksenAlkamiskausi(result: JValue): Option[KoulutuksenAlkamiskausi] = {
    (
      ...
      (result \ "koulutuksenAlkamisvuosi").extractOpt[Int]
    ) match {
      case (Some(uri), Some(versio), Some(vuosi)) =>
        Some(
  ...
  def toOpetus(result: JValue, koulutustyyppi: Koulutustyyppi): Opetus = {
    val opetuskielis: Seq[String] = (result \ "opetuskielis" \ "uris")
    ...
    val opetuskieletKuvaus: Kielistetty = (result \ "kuvausKomoto" \ "LISATIETOA_OPETUSKIELISTA" \ "tekstis")
      .extractOpt[Map[String, String]]
      .map(k => toKieliMap(k))
      .getOrElse(Map())
    val opetusaikaKoodiUrit: Seq[String] =
      (result \ "opetusAikas" \ "uris").extract[Map[String, Int]].map(k => s"${k._1}#${k._2}").toSeq
    val opetusaikaKuvaus: Kielistetty = Map()
    val opetustapaKoodiUrit: Seq[String] =
      (result \ "opetusPaikkas" \ "uris").extract[Map[String, Int]].map(k => s"${k._1}#${k._2}").toSeq
    ...
    val suunniteltuKesto: Tuple2[Option[Int], Option[Int]] = {
      (
        (result \ "suunniteltuKestoTyyppi" \ "uri").extractOpt[String],
        (result \ "suunniteltuKestoArvo").extractOpt[String]
      ) match {
        case (Some(tyyppi), Some(arvo)) if tyyppi == "suunniteltukesto_01" && isInteger(arvo) =>
          (Some(arvo.toInt), None)
        case (Some(tyyppi), Some(arvo)) if tyyppi == "suunniteltukesto_01" && isDouble(arvo) =>
          yearDoubleToYearAndMonth(arvo)
        case (Some(tyyppi), Some(arvo)) if tyyppi == "suunniteltukesto_02" && isInteger(arvo) =>
          (None, Some(arvo.toInt))
        case _ => (None, None)
      }
    }
...
class MigrationService(organisaatioServiceImpl: OrganisaatioServiceImpl) extends MigrationHelpers {
  ...
  def parseKoulutusFromResult(
      result: JValue,
      komo: JValue,
      koulutuskoodi2koulutusala: (String, Int) => Seq[String]
  ): Koulutus = {
    val opetusTarjoajat = (result \ "opetusTarjoajat")
      .extract[List[String]]
      .map(oid => resolveOrganisationOidForKoulutus(OrganisaatioOid(oid)))
      .distinct
    ...
    val koulutusKoodiUri =
      s"${(result \ "koulutuskoodi" \ "uri").extract[String]}#${(result \ "koulutuskoodi" \ "versio").extract[Int]}"
    ...
    val tutikintonimikes = (result \ "tutkintonimikes" \ "uris")
      .extract[Map[String, Int]]
      .map { case (koodi, versio) =>
        s"$koodi#$versio"
      }
      .toList
    val koulutusalaKoodiUrit =
      koulutuskoodi2koulutusala(
        (result \ "koulutuskoodi" \ "uri").extract[String],
    ...
    def toValintakoe(obj: JObject): Option[(String, Valintakoe)] = {
      def toValintakoetilaisuus(obj: JValue, kieli: Kieli): Valintakoetilaisuus = {
        def toOsoite(obj: JValue, kieli: Kieli): Option[Osoite] = {
          ...
          val postinumeroUri = (obj \ "postinumero")
            .extractOpt[String]
            .map(_ + "#2") // TODO: Viimeisimmän koodiversion selvittäminen koodistopalvelusta
          Some(
            Osoite(
  ...
  def parseHakukohdeFromResult(result: JValue): Hakukohde = {
    ...
    val pohjakoulutusvaatimus = (result \ "hakukelpoisuusvaatimusUris")
      .extract[List[String]]
      .map(_.split("_").last)
      .map(versio => s"pohjakoulutusvaatimuskouta_$versio#1")

    val aloituspaikat = Aloituspaikat(
      lukumaara = (result \ "aloituspaikatLkm").extractOrElse[Int](0),
      ensikertalaisille = (result \ "ensikertalaistenAloituspaikat").extractOpt[Int]
    )

    Hakukohde(
      oid = Some(HakukohdeOid((result \ "oid").extract[String])),
      ...
      metadata = Some(
        HakukohdeMetadata( //TODO: Suurin osa hakukohteen kentistä pitäisi siirtää metadatan sisään!
      ...
      Some(
        KoulutuksenAlkamiskausi(
          ...
          koulutuksenAlkamisvuosi =
            (result \ "koulutuksenAlkamisVuosi").extractOpt[Int].map(vuosi => (vuosi + 1).toString)
        )
      )
