...
import fi.oph.kouta.util.MiscUtils.{isDIAlukiokoulutus, isToisenAsteenYhteishaku}
  ...
  def validate(hakukohde: Hakukohde, haku: Option[Haku], isOphPaakayttaja: Boolean, deps: Map[String, (Julkaisutila, Option[Koulutustyyppi], Option[ToteutusMetadata], Seq[String])], method: String) = {
    ...
    and(
      ...
      validateIfDefined[ToteutusMetadata](deps.get(toteutusOid).flatMap(_._3), metadata => and(
        ...
        validateIfTrue(metadata.tyyppi == Lk && !isDIAlukiokoulutus(koulutuksetKoodiUri),
