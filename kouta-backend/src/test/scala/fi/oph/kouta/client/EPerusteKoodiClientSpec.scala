...
class EPerusteKoodiClientSpec extends ScalatraFlatSpec with KoodistoServiceMock {
  ...
  "When koulutusKoodiUri query failed" should "return error status" in {
    ...
    koodiClient.getKoulutusKoodiUritForEPerusteFromCache(66L).left.get.getMessage should equal("Failure in eperuste-service for ePerusteId 66")
  }

  "When tutkinnonosa query failed" should "return error status" in {
    mockTutkinnonOsatFailure(66L)
    koodiClient.getTutkinnonosaViitteetAndIdtForEPerusteFromCache(66L).left.get.getMessage should equal("Failure in eperuste-service for tutkinnonosat by ePerusteId 66")
  }

  "When osaamisala query failed" should "return error status" in {
    mockOsaamisalaKoodiUritFailure(66L)
    koodiClient.getOsaamisalaKoodiuritForEPerusteFromCache(66L).left.get.getMessage should equal("Failure in eperuste-service for osaamisalat by ePerusteId 66")
  }
}