...
class EPerusteKoodiClientSpec extends ScalatraFlatSpec with KoodistoServiceMock {
  ...
  "Querying koulutusKoodiUrit for ePeruste" should "return koodiurit if ePeruste was valid" in {
    ...
    koodiClient.getKoulutusKoodiUritForEPeruste(11L) should equal(Right(Seq(KoodiUri("koulutus_371101", 1), KoodiUri("koulutus_371102", 1))))
    koodiClient.getKoulutusKoodiUritForEPeruste(123L) should equal(Right(Seq[KoodiUri]()))
    koodiClient.getKoulutusKoodiUritForEPeruste(100L) should equal(Right(Seq[KoodiUri]()))
    clearServiceMocks()
  ...
  "Querying tutkinnonosa viitteet and id:t for ePeruste" should "return viitteet and id:t if ePeruste was existing" in {
    ...
    koodiClient.getTutkinnonosaViitteetAndIdtForEPeruste(123L) should equal(Right(Seq((122L, 1234L))))
    koodiClient.getTutkinnonosaViitteetAndIdtForEPeruste(11L) should equal(Right(Seq[(Long, Long)]()))
    clearServiceMocks()
  ...
  "Querying osaamisalaKoodiurit for ePeruste" should "return koodiurit if ePeruste was existing" in {
    ...
    koodiClient.getOsaamisalaKoodiuritForEPeruste(11L) should equal(Right(Seq(KoodiUri("osaamisala_01", 1), KoodiUri("osaamisala_02", 1))))
    koodiClient.getOsaamisalaKoodiuritForEPeruste(123L) should equal(Right(Seq[KoodiUri]()))
    clearServiceMocks()
  ...
  "When cache data is expired or removed" should "data fetched to cache again" in {
    ...
    mockTutkinnonOsatByEPeruste(123L, Seq((125L, 1235L)))
    mockOsaamisalaKoodiUritByEPeruste(11L, Seq("osaamisala_03", "osaamisala_04"))
    koodiClient.getKoulutusKoodiUritForEPeruste(11L) should equal(Right(Seq(KoodiUri("koulutus_371107", 1), KoodiUri("koulutus_371108", 1))))
    koodiClient.getTutkinnonosaViitteetAndIdtForEPeruste(123L) should equal(Right(Seq((125L, 1235L))))
    koodiClient.getOsaamisalaKoodiuritForEPeruste(11L) should equal(Right(Seq(KoodiUri("osaamisala_03", 1), KoodiUri("osaamisala_04", 1))))
  }

  "When koulutusKoodiUri query failed" should "return error status" in {
    mockKoulutusKoodiUritForEPerusteFailure(66L)
    koodiClient.getKoulutusKoodiUritForEPeruste(66L).left.get.getMessage should equal("Failure in eperuste-service for ePerusteId 66")
  }

  "When tutkinnonosa query failed" should "return error status" in {
    mockTutkinnonOsatFailure(66L)
    koodiClient.getTutkinnonosaViitteetAndIdtForEPeruste(66L).left.get.getMessage should equal("Failure in eperuste-service for tutkinnonosat by ePerusteId 66")
  }

  "When osaamisala query failed" should "return error status" in {
    mockOsaamisalaKoodiUritFailure(66L)
    koodiClient.getOsaamisalaKoodiuritForEPeruste(66L).left.get.getMessage should equal("Failure in eperuste-service for osaamisalat by ePerusteId 66")
  }
}