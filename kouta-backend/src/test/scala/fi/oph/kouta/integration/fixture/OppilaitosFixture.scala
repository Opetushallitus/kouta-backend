...
trait OppilaitosFixture extends KoutaIntegrationSpec with AccessControlSpec {
  ...
  def oppilaitosService: OppilaitosService = {
    ...
    new OppilaitosService(SqsInTransactionServiceIgnoringIndexing, MockS3ImageService, new AuditLog(MockAuditLogger), organisaatioService)
  }
  ...
  def oppilaitos = TestData.JulkaistuOppilaitos.copy(oid = OrganisaatioOid(s"1.2.246.562.10.000${random.nextInt().abs}"))
