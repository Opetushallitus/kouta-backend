...
class KoulutusServiceValidationSpec extends BaseServiceValidationSpec[Koulutus] {
  ...
  val yoWithOid: Koulutus = yo.copy(oid = Some(KoulutusOid("1.2.246.562.13.00000000000000000123")))
  val amk: Koulutus       = AmkKoulutus
  val min: Koulutus       = MinKoulutus.copy(koulutustyyppi = Yo)

  val kkOpintokokonaisuusKoulutus: Koulutus =
    KkOpintokokonaisuusKoulutus.copy(oid = Some(KoulutusOid("1.2.246.562.13.133")))
  ...
  override def beforeEach(): Unit = {
    ...
    when(organisaatioService.getAllChildOidsAndKoulutustyypitFlat(AmkOid))
      .thenAnswer(
        Seq(AmkOid),
        Seq(Amk, AmmOpeErityisopeJaOpo, KkOpintojakso, KkOpintokokonaisuus, Erikoistumiskoulutus)
      )
    ...
    when(koodistoService.koodiUriExistsInKoodisto(KoulutuksenLisatiedotKoodisto, "koulutuksenlisatiedot_03#1"))
      .thenAnswer(itemFound)
    when(koodistoService.koodiUriExistsInKoodisto(KoulutuksenLisatiedotKoodisto, "koulutuksenlisatiedot_04#1"))
    ...
    when(
      koodistoService.koodiUriExistsInKoodisto(
        KoulutusalaKoodisto,
        "kansallinenkoulutusluokitus2016koulutusalataso2_020#1"
      )
    )
      .thenAnswer(itemFound)
    when(koodistoService.koodiUriExistsInKoodisto(OpintojenLaajuusyksikkoKoodisto, "opintojenlaajuusyksikko_2#1"))
      .thenAnswer(itemFound)
    when(koodistoService.koodiUriExistsInKoodisto(OpintojenLaajuusyksikkoKoodisto, "opintojenlaajuusyksikko_5#1"))
      .thenAnswer(itemFound)
    when(koodistoService.koodiUriExistsInKoodisto(OpintojenLaajuusyksikkoKoodisto, "opintojenlaajuusyksikko_6#1"))
      .thenAnswer(itemFound)
    when(koodistoService.koodiUriExistsInKoodisto(OpintojenLaajuusyksikkoKoodisto, "opintojenlaajuusyksikko_8#1"))
      .thenAnswer(itemFound)
    ...
    when(toteutusDao.getByKoulutusOid(kkOpintokokonaisuusKoulutus.oid.get, TilaFilter.onlyOlemassaolevat()))
      .thenAnswer(Seq[Toteutus]())

    when(organisaatioService.withoutOppilaitostyypit(anySeq[OrganisaatioOid], anySeq[String])).thenReturn(Seq())

    //ammatillisten validointi
    when(koodistoService.getKaannokset("koulutus_371101#1"))
      .thenAnswer(Right(Map(Fi -> "nimi", Sv -> "nimi sv")))
    ...
    when(ePerusteKoodiClient.getKoulutusKoodiUritForEPerusteFromCache(11L))
      .thenAnswer(Right(Seq(koodiUriFromString("koulutus_371101"))))
    when(ePerusteKoodiClient.getKoulutusKoodiUritForEPerusteFromCache(1234L))
      .thenAnswer(Right(Seq(koodiUriFromString("koulutus_371101"))))
    when(ePerusteKoodiClient.getKoulutusKoodiUritForEPerusteFromCache(122L))
      .thenAnswer(Right(Seq(koodiUriFromString("koulutus_371101"))))
  ...
  it should "Succeed when koulutusalaKoodiUrit not changed in modify operation, even though invalid koodiUrit" in {
    ...
    val nonChangedKkOj =
      kkOpintojaksoWithParams(
        koulutusalaKoodiUrit = Seq("kansallinenkoulutusluokitus2016koulutusalataso1_66"),
        oid = koulutusOid
      )
  ...
  it should "fail if wrong tutkintoonJohtavuus" in {
    failTutkintoonjohtavuusValidation(AmmKoulutus, johtaaTutkintoon = false)
    failTutkintoonjohtavuusValidation(yo, johtaaTutkintoon = false)
    failTutkintoonjohtavuusValidation(AmmTutkinnonOsaKoulutus, johtaaTutkintoon = true)
    failTutkintoonjohtavuusValidation(AmmOsaamisalaKoulutus, johtaaTutkintoon = true)
  ...
  it should "fail if the tyyppi of the metadata differs from the tyyppi of the koulutus" in {
    failsValidation(
      amk.copy(metadata = Some(amkKoulutusMetadata.copy(tyyppi = Yo))),
      "metadata.tyyppi",
  ...
  it should "fail if invalid metadata kuvaus for koulutustyyppi with optional kuvaus" in {
    ...
    passesValidation(
      amk.copy(metadata =
        Some(amkKoulutusMetadata.copy(opintojenLaajuusyksikkoKoodiUri = Some("opintojenlaajuusyksikko_2#1")))
      )
    )
    failsValidation(
      amk.copy(metadata =
        Some(
          amkKoulutusMetadata.copy(
            opintojenLaajuusyksikkoKoodiUri = Some("opintojenlaajuusyksikko_2#1"),
  ...
  it should "fail if invalid metadata kuvaus for koulutustyyppi with mandatory kuvaus" in {
    val invalidKuvaus = Map(Fi -> "kuvaus vain suomeksi", Sv -> "")
    val m = TuvaKoulutuksenMetatieto.copy(
      kuvaus = Map(),
      opintojenLaajuusNumero = Some(10),
  ...
  it should "fail if julkaistu 'muu' koulutus has at least one julkaistu toteutus whose opintojenlaajuusMin is not in the range specified in koulutus" in {
    val muuKoulutus1 = muuKoulutusWithParams(opintojenLaajuusNumeroMin = Some(10), opintojenLaajuusNumeroMax = Some(20))
      .copy(tila = Julkaistu)
  ...
  it should "fail when trying to remove tarjoaja that is attached to a toteutus" in {
    val oldOpintojaksoKoulutus =
      kkOpintojaksoWithParams(isAvoinKorkeakoulutus = Some(true), tarjoajat = List(HkiYoOid, YoOid))
    ...
    when(organisaatioService.getAllChildAndParentOidsWithKoulutustyypitFlat(HkiYoOid))
      .thenReturn((Seq(HkiYoOid, OphOid, ChildOid), Seq(Yo)))
  ...
  it should "fail when no tarjoajat for julkaistu non-julkinen kk-related koulutus" in {
    failsValidation(
      KkOpintokokonaisuusKoulutus.copy(julkinen = false, tarjoajat = List()),
      "tarjoajat",
      missingTarjoajatForNonJulkinenKoulutus
    )
    failsValidation(
      KkOpintojaksoKoulutus.copy(julkinen = false, tarjoajat = List()),
      "tarjoajat",
      missingTarjoajatForNonJulkinenKoulutus
    )
    failsValidation(
      ErikoistumisKoulutus.copy(julkinen = false, tarjoajat = List()),
      "tarjoajat",
      missingTarjoajatForNonJulkinenKoulutus
    )
  }
  ...
  it should "fail if invalid ammatillinen koulutus" in {
    failsValidation(AmmKoulutus.copy(koulutuksetKoodiUri = Seq()), "koulutuksetKoodiUri", missingMsg)
  }
  ...
  it should "fail to create Amm koulutus that has osaamistavoitteet defined" in {
    ...
    failsValidation(
      ammKoulutusWithOsaamistavoitteet,
      Seq(
  ...
  it should "fail to create AmmOsaamisala koulutus that has osaamistavoitteet defined" in {
    val ammOsaamisalaKoulutus = AmmOsaamisalaKoulutus.copy(
      metadata = Some(AmmOsaamisalaKoulutusMetadata.copy(osaamistavoitteet = Map(Fi -> "osaamistavoitteet")))
    )
  ...
  it should "fail to create osaamismerkkikoulutus that has osaamistavoitteet defined" in {
    val osaamismerkkikoulutus = VapaaSivistystyoOsaamismerkkiKoulutus.copy(
      metadata = Some(VapaaSivistystyoOsaamismerkkiKoulutusMetatieto.copy(osaamistavoitteet = Map(Fi -> "osaamistavoitteet")))
    )
