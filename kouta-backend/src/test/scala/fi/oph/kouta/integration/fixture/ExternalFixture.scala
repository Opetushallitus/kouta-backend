...
trait ExternalFixture extends KoutaIntegrationSpec with KoulutusFixture with ToteutusFixture with HakuFixture with HakukohdeFixture with ValintaperusteFixture with SorakuvausFixture {
  ...
  def createKoulutus(oid: String = "", muokkaaja: Option[UserOid] = None, tila: Option[Julkaisutila] = None, isMuokkaajaOphVirkailija: Option[Boolean] = None): Koulutus =
    yoKoulutus.copy(oid = if (oid.isEmpty) None else Some(KoulutusOid(oid)), muokkaaja = muokkaaja.getOrElse(OphUserOid), tila = tila.getOrElse(Julkaistu),
      metadata = Some(
  ...
  def createHaku(oid: String = "", muokkaaja: Option[UserOid] = None, tila: Option[Julkaisutila] = None, isMuokkaajaOphVirkailija: Option[Boolean] = None): Haku =
    haku.copy(oid = if (oid.isEmpty) None else Some(HakuOid(oid)), muokkaaja = muokkaaja.getOrElse(OphUserOid), tila = tila.getOrElse(Julkaistu),
      metadata = Some(
        haku.metadata.get.copy(isMuokkaajaOphVirkailija = Some(isMuokkaajaOphVirkailija.getOrElse(true)))))
  ...
  def createHakukohde(oid: String = "", muokkaaja: Option[UserOid] = None, tila: Option[Julkaisutila] = None, isMuokkaajaOphVirkailija: Option[Boolean] = None): Hakukohde =
    hakukohde.copy(toteutusOid = toteutusOidForHakukohde, hakuOid = hakuOidForHakukohde, valintaperusteId = valintaperusteIdForHakukohde,
      oid = if (oid.isEmpty) None else Some(HakukohdeOid(oid)), muokkaaja = muokkaaja.getOrElse(OphUserOid), tila = tila.getOrElse(Julkaistu),
      metadata = Some(hakukohde.metadata.get.copy(isMuokkaajaOphVirkailija = Some(isMuokkaajaOphVirkailija.getOrElse(true)))))
  ...
  def createSorakuvaus(id: String = "", muokkaaja: Option[UserOid] = None, tila: Option[Julkaisutila] = None, isMuokkaajaOphVirkailija: Option[Boolean] = None): Sorakuvaus =
    sorakuvaus.copy(id = if (id.isEmpty) None else Some(UUID.fromString(id)), muokkaaja = muokkaaja.getOrElse(OphUserOid), tila = tila.getOrElse(Julkaistu),
      metadata = Some(sorakuvaus.metadata.get.copy(isMuokkaajaOphVirkailija = Some(isMuokkaajaOphVirkailija.getOrElse(true)))))
  ...
  def doPut[E](request: E, sessionId: UUID = defaultSessionId): String = {
    request match {
      ...
      case toteutusRequest: ExternalToteutusRequest => put(ExternalToteutusPath, toteutusRequest.copy(toteutus = toteutusRequest.toteutus.copy(koulutusOid = koulutusOidForToteutus)), sessionId, oid)
      ...
      case valintaperusteRequest: ExternalValintaperusteRequest => put(ExternalValintaperustePath, valintaperusteRequest, sessionId, id(_).toString)
      case sorakuvausRequest: ExternalSorakuvausRequest => put(ExternalSorakuvausPath, sorakuvausRequest, sessionId, id(_).toString)
  ...
  def doGet[E](oidOrId: String, expected: E, sessionId: UUID = defaultSessionId): String = {
    expected match {
      case koulutus: Koulutus => get(KoulutusPath, oidOrId, sessionId, koulutus.copy(modified = Some(readKoulutusModified(oidOrId))))
      case toteutus: Toteutus => get(ToteutusPath, oidOrId, sessionId, toteutus.copy(koulutusOid = koulutusOidForToteutus, modified = Some(readToteutusModified(oidOrId))))
      case haku: Haku => get(HakuPath, oidOrId, sessionId, haku.copy(modified = Some(readHakuModified(oidOrId))))
      case hakukohde: Hakukohde => get(HakukohdePath, oidOrId, sessionId, tallennettuHakukohde(oidOrId).copy(muokkaaja = hakukohde.muokkaaja, tila = hakukohde.tila, modified = Some(readHakukohdeModified(oidOrId)),
        metadata = hakukohde.metadata))
      case valintaperuste: Valintaperuste => get(ValintaperustePath, oidOrId, sessionId, valintaperuste.copy(modified = Some(readValintaperusteModified(oidOrId))))
      case sorakuvaus: Sorakuvaus => get(SorakuvausPath, oidOrId, sessionId, sorakuvaus.copy(modified = Some(readSorakuvausModified(oidOrId))))
    }
  }
