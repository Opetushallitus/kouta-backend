...
class ToteutusServiceValidationSpec extends BaseValidationSpec[Toteutus] {
  ...
  override def beforeEach(): Unit = {
    ...
    when(koulutusKoodiClient.lukioDiplomiKoodiUriExists("moduulikoodistolops2021_kald3#1")).thenAnswer(itemFound)
    when(koulutusKoodiClient.koulutusKoodiUriOfKoulutustyypitExistFromCache(Seq(ammatillinenPerustutkintoKoulutustyyppiKoodiUri), validKoulutuksetKoodiUri)).thenAnswer(itemFound)
    when(koulutusKoodiClient.koulutusKoodiUriOfKoulutustyypitExistFromCache(Seq(ammatillinenPerustutkintoKoulutustyyppiKoodiUri), invalidKoulutuksetKoodiUri)).thenAnswer(itemNotFound)
