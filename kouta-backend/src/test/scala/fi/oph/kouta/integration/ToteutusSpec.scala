...
class ToteutusSpec extends KoutaIntegrationSpec
  ...
  it should "store and update unfinished toteutus" in {
    val unfinishedToteutus = Toteutus(muokkaaja = Some(TestUserOid), koulutusOid = KoulutusOid(koulutusOid), organisaatioOid = ChildOid, modified = None, kielivalinta = Seq(Fi), nimi = Map(Fi -> "toteutus"))
