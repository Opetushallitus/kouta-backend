...
sealed trait ToteutusSQL extends ToteutusExtractors with ToteutusModificationSQL with SQLHelpers {
  ...
  val selectToteutusSql =
    s"""select t.oid, t.koulutus_oid, t.tila, t.nimi, t.metadata, t.muokkaaja, t.organisaatio_oid, t.kielivalinta, m.modified
          from toteutukset t
          inner join (
  ...
  val selectToteutusListSql =
    s"""select distinct t.oid, t.koulutus_oid, t.nimi, t.tila, t.organisaatio_oid, t.muokkaaja, m.modified
          from toteutukset t
          inner join (
            select t.oid oid, greatest(
              max(lower(t.system_time)),
